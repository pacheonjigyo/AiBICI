...
export function BrandingMenu(): JSX.Element {
  ...
  const handleNaming = () => {
    window.location.href = "/ai-branding/create#/naming";

    setNaming(!naming);
  };

  const handleStory = () => {
    window.location.href = "/ai-branding/create#/story";

    setStory(!story);
  };

  const handleMission = () => {
    window.location.href = "/ai-branding/create#/mission";

    setMission(!mission);
  };

  const handleVision = () => {
    window.location.href = "/ai-branding/create#/vision";

    setVision(!vision);
  };

  const handleEssense = () => {
    window.location.href = "/ai-branding/create#/essense";

    setEssense(!essense);
  };

  const handleSlogan = () => {
    window.location.href = "/ai-branding/create#/slogan";

    setSlogan(!slogan);
  };

  const handleCoreValue = () => {
    window.location.href = "/ai-branding/create#/corevalue";

    setCoreValue(!coreValue);
  };

  const handleLogo = () => {
    window.location.href = "/ai-branding/create#/logo";

    setLogo(!logo);
  };

  const handleMarketing = () => {
    window.location.href = "/ai-branding/create#/marketing";

    setMarketing(!marketing);
  };

  const handleTrade = () => {
    window.location.href = "/ai-branding/create#/trade";

    setTrade(!trade);
  };

  const handleChallenge = () => {
    window.location.href = "/ai-branding/create#/challenge";
  ...
  return (
    <Box
      ...
      sx={{
        ...
        width: 300,
        height: commonStore.baseInfo.height - 106,
      ...
      >
        ...
        <ListItem disablePadding>
          <ListItemButton disabled>
            ...
            <Typography fontSize={14}>벤치마킹</Typography>
          </ListItemButton>
        </ListItem>
      </List>
    </Box>
