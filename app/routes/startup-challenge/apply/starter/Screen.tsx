...
export default function ChatScreen(): JSX.Element {
  const { commonStore, engineChatStore, engineDataStore } =
  ...
  const onSend = async () => {
    ...
    if (engineChatStore.chatData.prompt.includes("스토리")) {
      symbols = await engineChatStore.getChatResponse(
        `"${response}"를 요약할 수 있는 핵심 단어 두 가지만 알려줘`,
      );
    }
      ...
      content: (
        <>
          <Box sx={{}}>
            ...
            {engineChatStore.chatData.prompt.includes("스토리") ? (
              <>
                <Box
                  sx={{
                    mt: 3,
                ...
                >
                  ...
                  <Button
                    ...
                    onClick={async () => {
                      engineDataStore.setBrandInfo({
                        ...
                        symbols,
                        description: `the importance of ${symbols
                          .split(",")
                          .join(" and ")}`,
                        workOwn: 1,
                      });
                  ...
                  >
                    {engineDataStore.isEngineBusy ? (
                      ...
                      <>
                        <AutoAwesome /> &nbsp; 스토리 기반으로 브랜드 아이덴티티
                        자동생성하기
                      </>
                    )}
                  </Button>
