...
export class CanvasEditor {
  ...
  init = async (
    ...
    if (this.externalCanvas) {
      ...
      if (this.image.current.includes("data:image/svg")) {
        this.image.original = await this.getImageFromUrl(this.imageUrl, true);
      } else {
        this.image.original = await this.getImageFromUrl(
          this.image.current,
          false,
        );
      }
