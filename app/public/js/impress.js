  ...
  var libraryFactory = function (rootId) {
    ...
    var possibleRotations = function (coordinate) {
      ...
      for (var i = 0; i < orders.length; ++i) {
        rotations.push(coordinateToOrderedRotation(coordinate, orders[i]));
      }
    ...
    var translateRelative = function (relative, prevRotation) {
      ...
      result.rotate = combineRotations([prevRotation, relative.rotate]);
...
(function (document, window) {
  ...
  var computeRelativePositions = function (el, prev) {
    ...
    if (data.relTo) {
      ...
      if (ref) {
        ...
        if (
          ...
          var prevPosition =
            ref.getAttribute("data-rel-position") || "absolute";
    ...
    if (step.relative.position === "relative" && inheritRotation) {
      ...
      relative = api.lib.rotation.translateRelative(step.relative, prev.rotate);
          ...
          for (var i = 0; i < attrs.length; i++) {
            if (step[attrs[i][1]] !== null) {
              if (step[attrs[i][0]] === null) {
                ...
                step.el.setAttribute("data-" + attrs[i][0], step[attrs[i][0]]);
              }
            }
          }
        }
